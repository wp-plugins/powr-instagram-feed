<?php
     ...
     */
    /*
    Plugin Name: POWr Instagram Feed
    Plugin URI: http://www.powr.io
    Description: Create a custom Instagram feed in seconds! Add the widget to your theme, or create an Instagram Feed on ANY page or post by using the shortcode [powr-instagram-feed]. Then, simply visit your site and click the settings icon to customize your Instagram Feed right in the page. Many more plugins & tutorials at POWr.io.
    Author: POWr.io
    Version: 1.1
    ...
    */
    ...
    class Powr_Instagram_Feed extends WP_Widget{
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_instagram_feed',
                             __( 'POWr Instagram Feed' ),
                             array( 'description' => __( 'Instagram Feed by POWr.io') )
        );
      }
      //This prints the div
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-instagram-feed' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ?>
        <p>
          No need to edit here - just click the gears icon on your Instagram Feed.
        </p>
        <p>
    ...
    function register_powr_instagram_feed() {
      register_widget( 'Powr_Instagram_Feed' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_instagram_feed' );
    //Create short codes for adding plugins anywhere:
    function powr_instagram_feed_shortcode( $atts ){
      if(isset($atts['label'])){
        $label = $atts['label'];
      }else{
        $label = '';
      }
      return "<div class='powr-instagram-feed' label='$label'></div>";
    }
    add_shortcode( 'powr-instagram-feed', 'powr_instagram_feed_shortcode' );
  ?>